...
function renderTodoListThree(){
    let todoListHTML = '';
    todoListThree.forEach((todoObject, index) => {
    ...
    document.querySelector(".js-todo-list-date")
        .innerHTML = todoListHTML;
    // html is now ON the page, so we will implement onClick behavior now
    
    // querySelectorAll returns a NodeList object => index: element
    // forEach takes element parameter first and index of that element
    document.querySelectorAll(".js-delete-todo-button")
        .forEach((deleteButton, index) => {
            deleteButton.addEventListener('click', () => {
                todoListThree.splice(index, 1);
                renderTodoListThree();
                saveToStorage();
            });
        });
